...
import (
	...
	"github.com/topgate/gcim-temporary/back/app/batch/internal/controllers"
	"github.com/topgate/gcim-temporary/back/app/batch/internal/initialize"
...
func main() {
	...
	openapi.RegisterHandlersWithBaseURL(e, si, "/batch/api")
	...
	fmt.Printf("http://localhost:%s/batch/api/health\n", externalPort)
...
func useAuthenticationMiddleware(e *echo.Echo, cfg config.Config, usecaseDependencies initialize.UseCaseDependencies) {
	param := authenticationMiddleware.NewAuthenticatorParam{
		...
		CookieSessionName: "gcim_api_batch_session", // バッチなので認証方法は別で検討
		SkipPaths: []string{
			"/batch/api/health",
		},
		ContextKey:            "job",
